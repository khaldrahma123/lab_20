...
int compareTreeItemByLevel(const void *treeItemPtr1, const void 
*treeItemPtr2) {
...
void medianFilter(int *matrix, int filter, int n, int m) {
 ...
 value = calculateMedianVector(matrix, m, row, col, arrayOfNums, 
filter);
...
// Функция для обработки данных и создания массива доменов счетчиков для
каждого поддомена
DomainCounter *subdomainVisits(char **cpdomains, int cpdomainsSize, int 
*returnSize) {
 // Создание массива структур DomainCounter
 DomainCounter *domainCounters = (DomainCounter 
*)malloc(sizeof(DomainCounter) * 1000);
 int size = 0;
 ...
 qsort(domainCounters, size, sizeof(DomainCounter), 
compareDomainCounters);
...
vectorVoid showVisitStats(char *stats) {
 ...
 idxEnd = idxEnd - strlen(STATS_END) + 1; // Учитываем длину конечной
метки
 ...
 qsort(data.data, data.size, sizeof(DomainRecord), 
compareDomainCounterNames);
 ...
 strcpy(domainCounter.domain, prevDomain.domain.begin); // Копируем
домен
...
void maxThreeImpl(int *nums, int level, int start, int end, int type, int 
*idx, vectorVoid *res) {
 (*idx)++;
 if (end < start) {
